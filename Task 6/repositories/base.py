...
class BaseRepository:
    ...
    async def edit(self, data: BaseModel, **filter_by) -> None:
        edit_data_stmt = update(self.model).filter_by(**filter_by).values(**data.model_dump())  # type: ignore
        print(
            edit_data_stmt.compile(engine, compile_kwargs={"literal_binds": True})
        )  # * Вывод сырого SQL запроса
        await self.session.execute(edit_data_stmt)

    async def delete(self, **filter_by) -> None:
        delete_data_stmt = delete(self.model).filter_by(**filter_by)  # type: ignore
        print(
            delete_data_stmt.compile(engine, compile_kwargs={"literal_binds": True})
        )  # * Вывод сырого SQL запроса
